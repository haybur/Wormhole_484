...
void mapXY() {
	...
	for (int i = 0; i < height; i++) { // On every row...
		for (int j = 0; j < 44; j++) { // For every column...
	...
	for (int i = 0; i < height; i++) { // On every row...
		for (int j = 0; j < 44; j++) { // For every column...
			if (i % 2 == 0) { // If it's going towards the right
				//coords[44 + i][j] = index; // The LED we're on is applied to that (x,y) coordinate
				coords[87 - j][height - 1 - i] = index;
				index++; // move to next LED
			}
			else { // If it's going towards the left...
				coords[44 + j][height - 1 - i] = index; // The LED we're on is applied to that (x,y) coordinate
				index++;
			}
		}
	}
...
void testMap() {
	for (int i = 0; i < width; i++) {
		for (int j = 0; j < height; j++) {
			Serial.println(coords[i][j]);
			leds[coords[i][j]] = CHSV(0, 255, 255);
			delay(10);
			LEDS.show();
			fadeAll(250);
		}
