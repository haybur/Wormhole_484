...
void addSparkles(int sparkStart, int sparkEnd, uint8_t freq, double multiplierAdjustment, int additionAdjustment, int hue, bool white) {
	...
	if (freq > 0 && freq < 15) {
		...
		for (int i = 0; i < numOfSparkles; i++) {
			...
			leds[pos] = ColorFromPalette(currentPalette, hue, sparkleBrightness);
		}
	}
...
void fadeAll(int fadeVar) {
	for (int i = 0; i < NUM_STRIPS * NUM_LEDS_PER_STRIP; i++) leds[i].nscale8(fadeVar);
}
void fadeAll2(int fadeVar) {
	for (int i = 0; i < NUM_STRIPS * NUM_LEDS_PER_STRIP; i++) leds2[i].nscale8(fadeVar);
}
void fadeAll3(int fadeVar) {
	for (int i = 0; i < NUM_STRIPS * NUM_LEDS_PER_STRIP; i++) leds3[i].nscale8(fadeVar);
}
